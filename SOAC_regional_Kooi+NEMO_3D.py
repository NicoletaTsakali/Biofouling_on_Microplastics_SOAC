...
if __name__ == "__main__":     
    ...
    p.add_argument('-region', choices = ('NPSG','EqPac','Med','SO','SubAtl', 'PolAtl'), action = "store", dest = "region", help ='region where particles released')
    ...
    if region == 'NPSG':
        ...
        lat_release0 = np.tile(np.linspace(30,40,10),[10,1]) #(20,28,5),[5,1]) 
        lat_release = lat_release0.T 
        lon_release = np.tile(np.linspace(0,30,10),[10,1]) #(-140,-148,5),[5,1]) 
    elif region == 'SubAtl' :
        lat_release0 = np.tile(np.linspace(10,50,10),[10,1]) #(20,28,5),[5,1]) 
        lat_release = lat_release0.T 
        lon_release = np.tile(np.linspace(-60,-20,10),[10,1]) #(-140,-148,5),[5,1])
    elif region == 'PolAtl' :
        lat_release0 = np.tile(np.linspace(56,67,10),[10,1]) #(20,28,5),[5,1]) 
        lat_release = lat_release0.T 
        lon_release = np.tile(np.linspace(-52,2,10),[10,1]) #(-140,-148,5),[5,1]) 
    ...
    outfile = '/science-nfs-sys/vsm01/users/6245404/SOAC/Kooi3D_'+region+'_'+s+'_'+yr+'_0'+res+'res_'+str(round(simdays,2))+'days_'+str(secsdt)+'dtsecs_'+str(round(hrsoutdt,2))+'hrsoutdt'
